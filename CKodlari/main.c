...
int main() {
    ...
    // Sonsuz döngü ile baðlantýlarý kabul etme
    while (1) {
        ...
        if (bytesReceived > 0) {
            ...
            if (bytesSent == SOCKET_ERROR) {
                printf("Mesaj gönderilemedi. Hata: %d\n", WSAGetLastError());
            } else {
                printf("Sunucu: %s\n", response);
            }
            ...
            printf("Mesaj alýnýrken hata oluþtu. Hata: %d\n", WSAGetLastError());
        }

        // Ýstemci ile iletiþim tamamlandýðýnda soketi kapat
       // closesocket(clientSocket);
        printf("Ýstemci ile baðlantý kapatýldý.\n");
    }

    // Sunucu soketini kapatma (bu satýra gelmesi mümkün deðil çünkü sonsuz döngüdeyiz)
    //closesocket(serverSocket);
    WSACleanup();
